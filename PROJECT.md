...
**重要**: 这个独立仓库不再依赖 `/geb-docs` 工作流。文档维护以仓库内的演化分析 skeleton 和实际代码变更为准。

1. 代码变更后检查对应的 `docs/geb/` 或 `docs/plans/` 文档
2. 更新影响的所有镜像文档节点
3. 优先保持知识图谱导出、前端说明、benchmark 文档三者一致
4. 如需做一致性检查，可手动运行相关 repo-local tests，而不是依赖主仓库自动流程
