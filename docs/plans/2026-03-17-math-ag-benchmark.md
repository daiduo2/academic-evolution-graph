...
# Math.AG Benchmark (Historical Superseded Snapshot)

> Historical note: this 2026-03-17 snapshot was superseded by
> `docs/plans/2026-03-18-math-ag-benchmark.md` in PKG-AG-01.
> Use the 2026-03-18 document as the current Math.AG benchmark source of truth.
...
## Expected Relations (Benchmark Runner)

**仅适用于已进入 runner 的 object_continuity cases：**

- `ag-b1`, `ag-e2` 必须命中 `math_ag_object_continuity`

## Expected Non-Relations (Benchmark Runner)

- `ag-n1` 不能因为共享 `projective` 泛词就误判

---

## Test Evidence Expectations (Not Benchmark Runner)

以下期望仅用于验证 `math_ag_method_continuity` 阈值逻辑，**不进入 benchmark runner**：

### Method Continuity Test Evidence

- `ag-method-p1`, `ag-method-p2` 在测试验证中命中 `math_ag_method_continuity`
- `ag-method-n1` 正确拒绝（仅共享单个 `cohomology`）

**注意**: 这些 cases 是 bridge-level，时间跨度不足，仅用于阈值验证。
