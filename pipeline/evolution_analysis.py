...
MATH_CO_MATROID_FAMILY_HINTS = {
    "matroid", "matroids", "polymatroid", "polymatroids",
}
MATH_CO_DISCRETE_STRUCTURE_HINTS = {
    "matroid", "matroids", "polymatroid", "polymatroids",
    "poset", "posets",
    "polytope", "polytopes",
    "lattice", "lattices",
}
MATH_CO_DISCRETE_STRUCTURE_FAMILIES = {
    "matroid": "matroid",
    "matroids": "matroid",
    "polymatroid": "matroid",
    "polymatroids": "matroid",
    "poset": "poset",
    "posets": "poset",
    "polytope": "polytope",
    "polytopes": "polytope",
...
MATH_DS_ENTROPY_CORE_HINTS = {
    "entropy",
}
MATH_DS_ENTROPY_FOCUS_HINTS = {
    "ergodic",
    "topological",
}
MATH_DS_ENTROPY_SUPPORTING_HINTS = {
    "measure",
    "measures",
}
MATH_NA_KRYLOV_CORE_HINTS = {
    "gmres",
    "krylov",
    "lanczos",
    "arnoldi",
}
MATH_NA_KRYLOV_SUPPORTING_HINTS = {
    "block",
    "matrix",
    "matrices",
    "eigenvalue",
...
def _canonical_term_families(terms: Iterable[str], family_map: Dict[str, str]) -> List[str]:
    return sorted({family_map[term] for term in terms if term in family_map})
...
def _build_pipeline_relation(anchor: TopicRecord, target: TopicRecord) -> Dict[str, object]:
    ...
    math_co_domain = (
        any("math.CO研究" in segment for segment in anchor.hierarchy_path)
        and any("math.CO研究" in segment for segment in target.hierarchy_path)
    )
    math_ds_domain = (
        any("math.DS研究" in segment for segment in anchor.hierarchy_path)
        and any("math.DS研究" in segment for segment in target.hierarchy_path)
    )
    math_na_domain = (
        any("math.NA研究" in segment for segment in anchor.hierarchy_path)
        and any("math.NA研究" in segment for segment in target.hierarchy_path)
    )
    math_lo_domain = (
    ...
    shared_math_co_matroid_terms = sorted((anchor_terms & target_terms) & MATH_CO_MATROID_FAMILY_HINTS)[:6]
    shared_math_co_discrete_terms = sorted((anchor_terms & target_terms) & MATH_CO_DISCRETE_STRUCTURE_HINTS)[:8]
    shared_math_ds_core_terms = sorted((anchor_terms & target_terms) & MATH_DS_ENTROPY_CORE_HINTS)[:4]
    shared_math_ds_focus_terms = sorted((anchor_terms & target_terms) & MATH_DS_ENTROPY_FOCUS_HINTS)[:4]
    shared_math_ds_supporting_terms = sorted((anchor_terms & target_terms) & MATH_DS_ENTROPY_SUPPORTING_HINTS)[:4]
    anchor_math_na_core_terms = sorted(anchor_terms & MATH_NA_KRYLOV_CORE_HINTS)[:6]
    target_math_na_core_terms = sorted(target_terms & MATH_NA_KRYLOV_CORE_HINTS)[:6]
    shared_math_na_core_terms = sorted((anchor_terms & target_terms) & MATH_NA_KRYLOV_CORE_HINTS)[:6]
    shared_math_na_supporting_terms = sorted((anchor_terms & target_terms) & MATH_NA_KRYLOV_SUPPORTING_HINTS)[:6]
    shared_math_co_matroid_families = _canonical_term_families(
        shared_math_co_matroid_terms,
        MATH_CO_DISCRETE_STRUCTURE_FAMILIES,
    )
    shared_math_co_discrete_families = _canonical_term_families(
        shared_math_co_discrete_terms,
        MATH_CO_DISCRETE_STRUCTURE_FAMILIES,
    )
    ...
    # Taxonomy overlap remains useful diagnostic output, but it is too loose to
    # independently establish Math.AG object continuity.
    ...
    elif math_co_domain and shared_math_co_matroid_families and len(shared_math_co_discrete_families) >= 2:
        relation = "math_co_matroid_structure_continuity"
    elif math_ds_domain and shared_math_ds_core_terms and shared_math_ds_focus_terms:
        relation = "math_ds_ergodic_entropy_continuity"
    # Keep the first math.NA gate narrow: exact Krylov-family terms can promote,
    # while matrix/block vocabulary is exposed only as supporting evidence.
    elif math_na_domain and (
        len(shared_math_na_core_terms) >= 2
        or (
            "gmres" in shared_math_na_core_terms
            and len(target_math_na_core_terms) >= 4
            and any(term in {"krylov", "lanczos", "arnoldi"} for term in target_math_na_core_terms)
        )
    ):
        relation = "math_na_krylov_iterative_continuity"
    ...
    return {
        ...
        "shared_math_co_matroid_terms": shared_math_co_matroid_terms,
        "shared_math_co_matroid_families": shared_math_co_matroid_families,
        "shared_math_co_discrete_terms": shared_math_co_discrete_terms,
        "shared_math_co_discrete_families": shared_math_co_discrete_families,
        "shared_math_ds_core_terms": shared_math_ds_core_terms,
        "shared_math_ds_focus_terms": shared_math_ds_focus_terms,
        "shared_math_ds_supporting_terms": shared_math_ds_supporting_terms,
        "anchor_math_na_core_terms": anchor_math_na_core_terms,
        "target_math_na_core_terms": target_math_na_core_terms,
        "shared_math_na_core_terms": shared_math_na_core_terms,
        "shared_math_na_supporting_terms": shared_math_na_supporting_terms,
...
def build_consistency_check(
    ...
    if pipeline_relation in {
        ...
        "math_na_krylov_iterative_continuity",
...
def select_anchor_topics(
    ...
    for topic_id, topic in topics.items():
        ...
        if not start_period:
            continue
        if category_filter and topic.category != category_filter:
            continue
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Filter export to specific category (e.g., 'math'). Empty means no filter.",
    )
    return parser.parse_args()


def _filter_graph_by_category(graph: Dict, category: str) -> Dict:
    """Filter graph to only include topics and edges from specified category."""
    if not category:
        return graph

    # Filter topics by category
    filtered_topics = [t for t in graph["nodes"]["topics"] if t.get("category") == category]
    filtered_topic_ids = {t["id"] for t in filtered_topics}

    # Filter edges to only include edges between filtered topics
    filtered_edges = {
        "adjacent_to": [
            e for e in graph["edges"]["adjacent_to"]
            if e.get("source") in filtered_topic_ids and e.get("target") in filtered_topic_ids
        ]
    }
    ...
    return {
        ...
        "periods": graph.get("periods", []),
        "nodes": {
            "categories": [c for c in graph["nodes"]["categories"] if c.get("id") == category],
            "periods": graph["nodes"]["periods"],
...
def main() -> None:
    ...
    cases, details = analyze_evolution_cases(
        ...
        category_filter=args.category_filter or None,
    )

    # Filter topics and graph for report/output if category filter is specified
    output_topics = topics
    output_graph = graph
    if args.category_filter:
        output_topics = {tid: t for tid, t in topics.items() if t.category == args.category_filter}
        output_graph = _filter_graph_by_category(graph, args.category_filter)

    report = build_report(cases["cases"], cases["analysis_periods"], output_topics)
    write_outputs(args.output_dir, output_graph, cases, details, report)
